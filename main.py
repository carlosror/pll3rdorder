...
def is_number(s):
	'''
	returns True if s is an actual number and False otherwise
    '''
	try:
		float(s)
		return True
...
def scientific(number):
	'''
	Returns a string of number in scientific notation. E.g., scientific(2)
	returns '2.000e+00' and scientific(7.45) returns '7.450e+00'
	'''
...
def loopFilter(gamma,LoopBW,PM,CPGain,KVCO,P,Fout,Fref,R,T31):
	'''
	loopFilter() is the heart of the tool.
	It takes all user-entered loop parameters and returns the values of the
	loop filter components as floats, a list of frequency values, a list of 
	phase and magnitude response lists, and time response parameters as well
	'''
	Fcomp = float(Fref/R)
...
def readNoiseSpreadsheet(workbook):
	'''
	Reads noise spreadsheet uploaded by user. Returns each column of file as a list.
	'''
...
PFDCPNoise, XTALNoise, PrescalerNoise, VCONoise, SDNoise):
	'''
	Computes the output referred noise of each of the loop components.
	Returns each of them as a list.
	'''
...
def interpolate(t,y):
	'''
	Performs linear interpolation to generate 601 points between 100 Hz and 100 MHz.
	This is so we can perform more accurate phase error computations, which require
	us to integrate the total output-referred phase noise. The interpolated data is
	eventually fed to a Javascript function in noisePlot.html that performs the phase
	error computation.
	
	The vector y is first converted to dB, then the interpolation is done in the dB
	realm, then it's converted back to V^2 and returned.
	'''
	ydB = []#Convert y to dB
...
def defaultNoise():
	'''
	Default PLL noise contributors data, to be used if no noise file is loaded
	'''
	f=np.logspace(2,8,31)
