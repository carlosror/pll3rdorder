...
def loopFilter(gamma,LoopBW,PM,CPGain,KVCO,Fout,Fcomp,T31):
	...
	return C1/1e-9,C2/1e-9,C3/1e-9,R2/1e3,R3/1e3,f,magCL,magOL,phaseOL,magvcoTF,magprescalerTF,magpfdcpTF

def noiseContributors(workbook,magCL,magvcoTF,magprescalerTF,magpfdcpTF):
	...
	TotalNoise = []
	for i in range(sheetPFDCP.nrows):
		TotalNoise.append(20*np.log10(10**(PFDCPNoiseOut[i]/20.0) + 10**(PrescalerNoiseOut[i]/20.0) + 10**(VCONoiseOut[i]/20.0)))
		#TotalNoise.append(PFDCPNoiseOut[i] + PrescalerNoiseOut[i] + VCONoiseOut[i])
	return PFDCPNoiseOut,PrescalerNoiseOut,VCONoiseOut,TotalNoise
...
class MainHandler(webapp2.RequestHandler):
	...
	def post(self):
		...
		C1,C2,C3,R2,R3,f,magCL,magOL,phaseOL,magvcoTF,magprescalerTF,magpfdcpTF = loopFilter(enteredGamma,enteredLoopBW,enteredPM,enteredKphi,enteredKVCO,enteredFout,enteredFcomp,enteredT31)
		...
		PFDCPNoiseOut,PrescalerNoiseOut,VCONoiseOut,TotalNoise = noiseContributors(workbook,magCL,magvcoTF,magprescalerTF,magpfdcpTF)
		template = jinja_environment.get_template('noisePlot.html')
		self.response.out.write(template.render(f=f,PFDCPNoiseOut=PFDCPNoiseOut,PrescalerNoiseOut=PrescalerNoiseOut,VCONoiseOut=VCONoiseOut,TotalNoise=TotalNoise,index2=index))
		#template = jinja_environment.get_template('loopResponse.html')
		#self.response.out.write(template.render(f=f,magCL=magCL,magOL=magOL,phaseOL=phaseOL,magvcoTF=magvcoTF,index2=index))
