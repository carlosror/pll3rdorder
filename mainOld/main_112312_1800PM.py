...
def noiseContributors(workbook,magCL,magvcoTF,magprescalerTF,magpfdcpTF,R2,magLFTFR2,R3,magLFTFR3, magXTAL):
	...
	if workbook == "":#If no file is uploaded or there's an error with the file
		...
		SDNoiseOut = []
		for i in range(len(magCL)):
			SDNoise.append(sheetSD.cell(i,1).value)
			SDNoiseOut.append(SDNoise[i] + magCL[i])
	TotalNoise = []
	for i in range(len(magCL)):
	...
	return XTALNoiseOut, PFDCPNoiseOut,PrescalerNoiseOut,VCONoiseOut,R2NoiseOut,R3NoiseOut,SDNoiseOut, TotalNoise
...
class MainHandler(webapp2.RequestHandler):
	...
	def post(self):
		...
		XTALNoiseOut, PFDCPNoiseOut,PrescalerNoiseOut,VCONoiseOut,R2NoiseOut,R3NoiseOut,SDNoiseOut,TotalNoise = noiseContributors(workbook,magCL,magvcoTF,magprescalerTF,magpfdcpTF,(R2*1e3),magLFTFR2,(R3*1e3),magLFTFR3, magXTAL)
