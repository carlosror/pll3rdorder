...
def loopFilter(gamma,LoopBW,PM,CPGain,KVCO,Fout,Fcomp,T31):
	...
	return C1/1e-9,C2/1e-9,C3/1e-9,R2/1e3,R3/1e3,f,magCL,magOL,phaseOL,magvcoTF,magprescalerTF,magpfdcpTF,magLFTFR2

def noiseContributors(workbook,magCL,magvcoTF,magprescalerTF,magpfdcpTF,R2,magLFTFR2):
	...
	return PFDCPNoiseOut,PrescalerNoiseOut,VCONoiseOut,R2NoiseOut,TotalNoise
...
class MainHandler(webapp2.RequestHandler):
	...
	def post(self):
		...
		C1,C2,C3,R2,R3,f,magCL,magOL,phaseOL,magvcoTF,magprescalerTF,magpfdcpTF,magLFTFR2 = loopFilter(enteredGamma,enteredLoopBW,enteredPM,enteredKphi,enteredKVCO,enteredFout,enteredFcomp,enteredT31)
		...
		PFDCPNoiseOut,PrescalerNoiseOut,VCONoiseOut,R2NoiseOut,TotalNoise = noiseContributors(workbook,magCL,magvcoTF,magprescalerTF,magpfdcpTF,(R2*1e3),magLFTFR2)
		template = jinja_environment.get_template('noisePlot.html')
		self.response.out.write(template.render(f=f,PFDCPNoiseOut=PFDCPNoiseOut,PrescalerNoiseOut=PrescalerNoiseOut,VCONoiseOut=VCONoiseOut,R2NoiseOut=R2NoiseOut,TotalNoise=TotalNoise,index2=index))
		#template = jinja_environment.get_template('loopResponse.html')
		#self.response.out.write(template.render(f=f,magCL=magCL,magOL=magOL,phaseOL=phaseOL,magvcoTF=magvcoTF,index2=index))
