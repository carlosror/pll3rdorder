...
def loopFilter(gamma,LoopBW,PM,CPGain,KVCO,P,Fout,Fref,R,T31):
	...
	t, fT = timeResponse(A2,A1,A0,T2,Fcomp,Fout,LoopBW,KVCO,CPGain)
	return C1/1e-9,C2/1e-9,C3/1e-9,R2/1e3,R3/1e3,f,magCL,magOL,phaseOL,magvcoTF,magprescalerTF,magpfdcpTF,magLFTFR2,magLFTFR3, magXTAL, t, fT
...
def timeResponse(A2,A1,A0,T2,Fcomp,Fout,LoopBW,KVCO,CPGain):
	...
	t=np.linspace(0,8.0/LoopBW/1.0,10000)
	fT = []
	B0 = []
	B1 = []
	B2 = []
	B3 = []
	for i in range(len(t)):
		B0.append(B[0]*math.exp(p[0]*t[i])*((1/p[0])+T2))
		B1.append(B[1]*math.exp(p[1]*t[i])*((1/p[1])+T2))
		B2.append(B[2]*math.exp(p[2]*t[i])*((1/p[2])+T2))
		B3.append(B[3]*math.exp(p[3]*t[i])*((1/p[3])+T2))
	...
	LT_0p000001Pcnt = 0.00
	Tol_0p000001Pcnt = (0.000001/100.00)*f2
	for i in range(len(t)):
		if abs(fT[i] - f2)<Tol_0p001Pcnt and LT_0p001Pcnt==0.00:
			LT_0p001Pcnt = t[i]
			# print  LT_0p01Pcnt
		if abs(fT[i] - f2)<Tol_0p0001Pcnt and LT_0p0001Pcnt==0.00:
			LT_0p0001Pcnt = t[i]
		if abs(fT[i] - f2)<Tol_0p00001Pcnt and LT_0p00001Pcnt==0.00:
			LT_0p00001Pcnt = t[i]
		if abs(fT[i] - f2)<Tol_0p000001Pcnt and LT_0p000001Pcnt==0.00:
			LT_0p000001Pcnt = t[i]
	# print Tol_0p001Pcnt, LT_0p001Pcnt
	# print Tol_0p0001Pcnt, LT_0p0001Pcnt
	# print Tol_0p00001Pcnt, LT_0p00001Pcnt
	# print Tol_0p000001Pcnt, LT_0p000001Pcnt
	return t, fT
...
class MainHandler(webapp2.RequestHandler):
	...
	def post(self):
		...
		C1,C2,C3,R2,R3,f,magCL,magOL,phaseOL,magvcoTF,magprescalerTF,magpfdcpTF,magLFTFR2,magLFTFR3, magXTAL, t, fT = loopFilter(enteredGamma,enteredLoopBW,enteredPM,enteredKphi,enteredKVCO,enteredP,enteredFout,enteredFref,enteredR,enteredT31)
		index=range(1,len(f))
		try:
		...
		if (self.request.get("excelResults") == "wantExcelResults"):
			book2 = writeResults(C1*1e-9,C2*1e-9,C3*1e-9,R2*1e3,R3*1e3,f,magCL,magOL,phaseOL,magvcoTF,PFDCPNoiseOut,PrescalerNoiseOut,VCONoiseOut,R2NoiseOut,R3NoiseOut, XTALNoiseOut, SDNoiseOut, TotalNoise, t, fT)
