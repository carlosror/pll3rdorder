...
def noiseContributors(workbook,magCL,magvcoTF,magprescalerTF,magpfdcpTF,R2,magLFTFR2,R3,magLFTFR3):
	...
	if workbook == "":#If no file is uploaded or there's an error with the file
		...
		XTALNoiseOut = []
		for i in range(len(f)):
			XTALNoiseOut.append(XTALNoise[i] + magCL[i])
		PFDCPNoiseOut = []
		for i in range(len(f)):
...
class MainHandler(webapp2.RequestHandler):
	...
	def post(self):
		...
		XTALNoiseOut, PFDCPNoiseOut,PrescalerNoiseOut,VCONoiseOut,R2NoiseOut,R3NoiseOut,TotalNoise = noiseContributors(workbook,magCL,magvcoTF,magprescalerTF,magpfdcpTF,(R2*1e3),magLFTFR2,(R3*1e3),magLFTFR3)
