...
def loopFilter(gamma,LoopBW,PM,CPGain,KVCO,P,Fout,Fref,R,T31):
	...
	for i in range(len(f)):
		...
		magXTAL.append(magCL[i] - 20*np.log10(1.0))
	return C1/1e-9,C2/1e-9,C3/1e-9,R2/1e3,R3/1e3,f,magCL,magOL,phaseOL,magvcoTF,magprescalerTF,magpfdcpTF,magLFTFR2,magLFTFR3, magXTAL
...
def noiseContributors(workbook,magCL,magvcoTF,magprescalerTF,magpfdcpTF,R2,magLFTFR2,R3,magLFTFR3, magXTAL):
	...
	if workbook == "":#If no file is uploaded or there's an error with the file
		f, XTALNoise, PFDCPNoise, PrescalerNoise, VCONoise = defaultNoise()
		XTALNoiseOut = []
		for i in range(len(f)):
		...
		XTALNoiseOut = []
		for i in range(len(magCL)):
			XTALNoise.append(sheetXTAL.cell(i,1).value)
			XTALNoiseOut.append(XTALNoise[i] + magXTAL[i])
	...
	TotalNoise = []
	for i in range(len(magCL)):
		TotalNoise.append(10*np.log10(10**(XTALNoiseOut[i]/10.0) + 10**(PFDCPNoiseOut[i]/10.0) + 10**(PrescalerNoiseOut[i]/10.0) + 10**(VCONoiseOut[i]/10.0) + 10**(R2NoiseOut[i]/10.0) ))
		#TotalNoise.append(PFDCPNoiseOut[i] + PrescalerNoiseOut[i] + VCONoiseOut[i])	
	return XTALNoiseOut, PFDCPNoiseOut,PrescalerNoiseOut,VCONoiseOut,R2NoiseOut,R3NoiseOut,TotalNoise
...
def defaultNoise():
	...
	return f, XTALNoise, PFDCPNoise, PrescalerNoise, VCONoise
	
def writeResults(C1,C2,C3,R2,R3,f,magCL,magOL,phaseOL,magvcoTF,PFDCPNoiseOut,PrescalerNoiseOut,VCONoiseOut,R2NoiseOut,R3NoiseOut,TotalNoise):
	book = Workbook()
	...
	sheetLoopResponse.col(0).width = 5000
	sheetLoopResponse.write(0,0,'Frequency',columnHeader)
	sheetLoopResponse.col(1).width = 12000
	sheetLoopResponse.write(0,1,'Closed Loop Response Magnitude',columnHeader)
	sheetLoopResponse.col(2).width = 12000
	sheetLoopResponse.write(0,2,'Open Loop Response Magnitude',columnHeader)
	sheetLoopResponse.col(3).width = 12000
	sheetLoopResponse.write(0,3,'Open Loop Response Phase',columnHeader)
	sheetLoopResponse.col(4).width = 12000
	sheetLoopResponse.write(0,4,'VCO Transfer Function Magnitude',columnHeader)
	for i in range(len(f)):
	...
	sheetPLLNoise.col(0).width = 5000
	sheetPLLNoise.write(0,0,'Frequency',columnHeader)
	sheetPLLNoise.col(1).width = 5000
	sheetPLLNoise.write(0,1,'PFDCP',columnHeader)
	sheetPLLNoise.col(2).width = 5000
	sheetPLLNoise.write(0,2,'Prescaler',columnHeader)
	sheetPLLNoise.col(3).width = 5000
	sheetPLLNoise.write(0,3,'VCO',columnHeader)
	sheetPLLNoise.col(4).width = 5000
	sheetPLLNoise.write(0,4,'R2',columnHeader)
	sheetPLLNoise.col(5).width = 5000
	sheetPLLNoise.write(0,5,'R3',columnHeader)
	sheetPLLNoise.col(6).width = 5000
	sheetPLLNoise.write(0,6,'Total Noise',columnHeader)
	for i in range(len(f)):
		...
		sheetPLLNoise.write(i+1,6,TotalNoise[i],parameterValue2)
	return book
...
class MainHandler(webapp2.RequestHandler):
	...
	def post(self):
		...
		C1,C2,C3,R2,R3,f,magCL,magOL,phaseOL,magvcoTF,magprescalerTF,magpfdcpTF,magLFTFR2,magLFTFR3, magXTAL = loopFilter(enteredGamma,enteredLoopBW,enteredPM,enteredKphi,enteredKVCO,enteredP,enteredFout,enteredFref,enteredR,enteredT31)
		index=range(1,len(f))
		try:
		...
		XTALNoiseOut, PFDCPNoiseOut,PrescalerNoiseOut,VCONoiseOut,R2NoiseOut,R3NoiseOut,TotalNoise = noiseContributors(workbook,magCL,magvcoTF,magprescalerTF,magpfdcpTF,(R2*1e3),magLFTFR2,(R3*1e3),magLFTFR3, magXTAL)
		if (self.request.get("excelResults") == "wantExcelResults"):
			book2 = writeResults(C1*1e-9,C2*1e-9,C3*1e-9,R2*1e3,R3*1e3,f,magCL,magOL,phaseOL,magvcoTF,PFDCPNoiseOut,PrescalerNoiseOut,VCONoiseOut,R2NoiseOut,R3NoiseOut,TotalNoise)
			...
			self.response.out.write(template.render(f=f, XTALNoiseOut=XTALNoiseOut, PFDCPNoiseOut=PFDCPNoiseOut,PrescalerNoiseOut=PrescalerNoiseOut,VCONoiseOut=VCONoiseOut,R2NoiseOut=R2NoiseOut,R3NoiseOut=R3NoiseOut,TotalNoise=TotalNoise,index2=index,error=noiseError))
