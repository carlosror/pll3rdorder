...
def writeNoiseSources():
	...
	for i in range(100):
		sheetXTAL.row(i).set_style(whiteCell)#Make everything white first
	sheetXTAL.col(0).width = 6000
	sheetXTAL.col(1).width = 8000
	sheetXTAL.write(0,0,'Frequency (Hz)',columnHeaderBorderTLBR)
	sheetXTAL.write(0,1,'XTAL Noise (dBc/Hz)',columnHeaderBorderTLBR)
	sheetPFDCP = book.add_sheet('PFDCP')
	for i in range(100):
		sheetPFDCP.row(i).set_style(whiteCell)#Make everything white first
	sheetPFDCP.col(0).width = 6000
	sheetPFDCP.col(1).width = 8000
	sheetPFDCP.write(0,0,'Frequency (Hz)',columnHeaderBorderTLBR)
	sheetPFDCP.write(0,1,'PFDCP Noise (dBc/Hz)',columnHeaderBorderTLBR)
	sheetPrescaler = book.add_sheet('Prescaler')
	for i in range(100):
		sheetPrescaler.row(i).set_style(whiteCell)#Make everything white first
	sheetPrescaler.col(0).width = 6000
	sheetPrescaler.col(1).width = 10000
	sheetPrescaler.write(0,0,'Frequency (Hz)',columnHeaderBorderTLBR)
	sheetPrescaler.write(0,1,'Prescaler Noise (dBc/Hz)',columnHeaderBorderTLBR)
	sheetVCO = book.add_sheet('VCO')
	for i in range(100):
		sheetVCO.row(i).set_style(whiteCell)#Make everything white first
	sheetVCO.col(0).width = 6000
	sheetVCO.col(1).width = 8000
	sheetVCO.write(0,0,'Frequency (Hz)',columnHeaderBorderTLBR)
	sheetVCO.write(0,1,'VCO Noise (dBc/Hz)',columnHeaderBorderTLBR)
	sheetSD = book.add_sheet('Sigma Delta')
	for i in range(100):
		sheetSD.row(i).set_style(whiteCell)#Make everything white first
	sheetSD.col(0).width = 6000
	sheetSD.col(1).width = 10000
	sheetSD.write(0,0,'Frequency (Hz)',columnHeaderBorderTLBR)
	sheetSD.write(0,1,'Sigma Delta Noise (dBc/Hz)',columnHeaderBorderTLBR)
